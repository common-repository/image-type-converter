<?php
namespace GPLSCore\GPLS_PLUGIN_WICOR\Core;
...
class Core {
	...
	public function new_keyword( $title = 'new', $return = true ) {
		...
		?>
		<span style="color: #ffffff;background: #dcc2e5;border-radius: 5px;" class="<?php echo esc_attr( $this->plugin_info['classes_general'] . '-new-keyword' ); ?> ms-1"><?php esc_html_e( ! empty( $title ) ? $title : 'New' ); ?></span>
		<?php
		if ( $return ) {
			return ob_get_clean();
		}
